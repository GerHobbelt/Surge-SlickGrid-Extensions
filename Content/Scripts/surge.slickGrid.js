...
(function ($) {
	...
	var Define = Surge.Define,
		...
		applyFormat = $.format ? $.proxy($.format, $) : (window.Globalize && $.proxy(Globalize.format, Globalize)) ||
	...
	$(function () {
		// We want to generate an error if certain expected css is not loaded
		var dummyElement = $('<p>').addClass("surge-slickGrid-loaded-marker").appendTo("body");

		if (dummyElement.css("z-index") != -98256 && Console && Console.error) {
			Console.error("Could not find surge.slickGrid.css styles. Surge.slickGrid requires surge.slickGrid.css in order to function properly");
		}
		dummyElement.remove();
	});
	...
	Class("Surge.SlickGrid.JSONDataSource", Surge.Component, {
		...
		bind: function (target, targetCtx) {
			...
			if (grid.onViewportChanged && grid.onViewportChanged.subscribe)
				grid.onViewportChanged.subscribe($.proxy(this._onViewportChanged, this));
			else
				grid.onViewportChanged = $.proxy(this._onViewportChanged, this);
